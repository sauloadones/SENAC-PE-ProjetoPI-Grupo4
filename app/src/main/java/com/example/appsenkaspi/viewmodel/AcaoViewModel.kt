...
class AcaoViewModel(application: Application) : AndroidViewModel(application) {
    ...
    private val dao = AppDatabase.Companion.getDatabase(application).acaoDao()
  ...
  suspend fun gerarResumoDashboardDireto(pilarId: Int): ResumoDashboard =
      withContext(Dispatchers.IO) {
          ...
          val vencidas = AppDatabase.Companion.getDatabase(getApplication())
              .atividadeDao()
  ...
  fun gerarResumoDashboard(pilarId: Int?, callback: (ResumoDashboard) -> Unit) {
    viewModelScope.launch(Dispatchers.IO) {
      ...
      val vencidas = if (pilarId == null) {
        AppDatabase.Companion.getDatabase(getApplication()).atividadeDao().contarVencidasGeral()
      } else {
        AppDatabase.Companion.getDatabase(getApplication()).atividadeDao().contarVencidasPorPilar(pilarId)
      }
